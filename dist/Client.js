...
const createMessage_1 = __importDefault(require("./listeners/createMessage"));
...
class Client extends Eris.Client {
    constructor(options) {
        ...
        */
        ...
        if (options.disableDefaultInteractionListener !== undefined)
            this.disableDefaultInteractionListener = options.disableDefaultInteractionListener;
        ...
        if (!this.disableDefaultMessageListener) {
            this.addEvent(createMessage_1.default);
        }
        if (!this.disableDefaultInteractionListener) {
            this.addEvent(interactionCreate_1.default);
        }
    }
     ...
     * @private
     */
    _performFirstReadySetup() {
        return __awaiter(this, void 0, void 0, function* () {
            // Generate mention regexp with user ID
            this.mentionPrefixRegExp = new RegExp(`^<@!?${this.user.id}>\\s?`);
            // Get OAuth application
            this.app = yield this.getOAuthApplication();
            // Get our application commands
            const guildCommands = yield this.getGuildCommands('149327211470520321');
            // Delete all our commands
            yield Promise.all(guildCommands.map((command) => __awaiter(this, void 0, void 0, function* () {
                if (command.application_id !== this.app.id) {
                    return;
                }
                yield this.deleteGuildCommand('149327211470520321', command.id);
            })));
            // Write all the updated commands
            yield Promise.all(this.registeredApplicationCommands.map((command) => __awaiter(this, void 0, void 0, function* () {
                const { id } = yield this.createGuildCommand('149327211470520321', command.toJSON());
     ...
     * returns `false`.
     */
    processApplicationCommandResponse(interaction) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    addFile(filename) {
        ...
        for (let thing of things) {
            ...
            try {
                ...
                else if (thing instanceof SlashCommand_1.SlashCommand) {
